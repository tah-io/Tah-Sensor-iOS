...
- (void)viewDidLoad
{
    ...
    MoistureSensorUpdatetimer = [NSTimer scheduledTimerWithTimeInterval:2.0 target:self selector:@selector(MoistureSensorUpdate:)
                                                           userInfo:nil
                                                            repeats:YES];
}
...
{
    [MoistureSensorUpdatetimer invalidate];
}
...
-(void)MoistureSensorUpdate:(NSTimer *)timer
{
    connectedsensorpin = (sensorpinsegment.selectedSegmentIndex + 410.0); // Defines Sensor Connected Pin
    
    if(sensor.activePeripheral.state)
    {
        [sensor getTAHSoilMoistureSensorUpdate:sensor.activePeripheral AnalogPin:connectedsensorpin];
    }
...
-(void) TAHbleCharValueUpdated:(NSString *)UUID value:(NSData *)data
{
    ...
    [self valuetomoisture];  // update distance
    }
...
-(void)valuetomoisture
{
    int rawmoisture = [rawDataString intValue];
    moisturelabel.text = [NSString stringWithFormat:@"%d%@",rawmoisture,@"%"];
}
...
- (IBAction)settings:(id)sender {
    ...
    else
    {
        ...
        [self valuetomoisture];  // Update if any change in  Distance Unit Type
        
        ///////// Wind Sensor Update Timer //////////
        
        MoistureSensorUpdatetimer = [NSTimer scheduledTimerWithTimeInterval:2.0
                                                                 target:self
                                                               selector:@selector(MoistureSensorUpdate:)
                                                               userInfo:nil
                                                                repeats:YES];
    }
}
