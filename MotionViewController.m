...
- (void)viewDidLoad
{
    ...
    MotionSensorUpdatetimer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(MotionSensorUpdate:)
                                                            userInfo:nil
                                                             repeats:YES];
}
...
{
    [MotionSensorUpdatetimer invalidate];
}
...
-(void) TAHbleCharValueUpdated:(NSString *)UUID value:(NSData *)data
{
    ...
    [self valuetoMotion];  // Update Touch Status
    }
    
}


-(void)MotionSensorUpdate:(NSTimer *)timer
{
    connectedsensorpin = (sensorpinsegment.selectedSegmentIndex + 2.0); // Defines Sensor Connected Pin
    
    if(sensor.activePeripheral.state)
    {
        [sensor getTAHPIRMotionSensorUpdate:sensor.activePeripheral SensorPin:connectedsensorpin];
    }
}
...
-(void)valuetoMotion
{
    ...
    int rawmotion = [rawDataString intValue];
    
    if (rawmotion == 1)
    {
       
        
        MotionStatusLabel.text = @"Motion Detected";
    ...
    else
    {
        ...
        MotionStatusLabel.text = @"No Motion";
    }
...
- (IBAction)settings:(id)sender {
    ...
    else
    {
        ...
        [self valuetoMotion];  // Update if any change in  Distance Unit Type
        
        ///////// Temperature Sensor Update Timer //////////
        
        MotionSensorUpdatetimer = [NSTimer scheduledTimerWithTimeInterval:1.0
                                                                  target:self
                                                                selector:@selector(MotionSensorUpdate:)
                                                                userInfo:nil
                                                                 repeats:YES];
    }
}
